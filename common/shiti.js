...
function initShiti(shiti) {
  let leibie = shiti.leibie;

  //给试题设置章idx 节idx 和默认已做答案等
  shiti.isAnswer = false;

  if (leibie == '1' || leibie == '3') { //单选和判断
    shiti.tx = "单选题"
...
function setMarkAnswer(shiti, isModelReal, isSubmit, self) {
  ...
  markAnswerItems[px - 1].select = shiti.leibie;
  ...
  switch (shiti.leibie) {
    ...
    case '3':
      ...
      if (done_daan != shiti.answer) {
        srcs[done_daan] = "/images/error_Single.png" //如果答错就把当前图标变为错误图标
        flag = 1;
      } else {
        srcs[done_daan] = "/images/right_Single.png";
        flag = 0;
      }
    ...
    case '2':
      ...
      for (let i = 0; i < answers.length; i++) {
        shiti.srcs[answers[i]] = "/images/right_Multi-1.jpg";
      }

      for (let i = 0; i < done_daan.length; i++) {
        if (answers.indexOf(done_daan[i]) >= 0) { //如果正确答案包含选中
          shiti.srcs[done_daan[i]] = "/images/right_Multi.png";
        } else {
          shiti.srcs[done_daan[i]] = "/images/error_Multi.png";
        }
      }
      /**
  ...
  switch (shiti.leibie) {
    ...
    case "2":
      ...
      for (let i = 0; i < new_done_daan.length; i++) {
        shiti.srcs[new_done_daan[i]] = "/images/right_Multi.png";
      }
...
function changeMultiShiti(done_daan, shiti) {
  ...
  for (let i = 0; i < done_daan.length; i++) {
    shiti.srcs[done_daan[i]] = "/images/right_Multi.png"; //将所有选中的选项置位正确图标
  }
}
 ...
 * 模拟真题中点击答题板后的处理方法
 */
function processTapModelRealAnswer(midShiti, preShiti, nextShiti, px, current, circular, shitiArray, self) {
  let myCurrent = current;

  let sliderShitiArray = [];

  initShiti(midShiti, self); //初始化试题对象
  processModelRealDoneAnswer(midShiti.done_daan, midShiti, self);

  if (px != 1 && px != shitiArray.length) { //如果不是第一题也是不是最后一题
    preShiti = shitiArray[px - 2];
    initShiti(preShiti, self); //初始化试题对象
    processModelRealDoneAnswer(preShiti.done_daan, preShiti, self);
    nextShiti = shitiArray[px];
    initShiti(nextShiti, self); //初始化试题对象
    processModelRealDoneAnswer(nextShiti.done_daan, nextShiti, self);
  } else if (px == 1) { //如果是第一题
    nextShiti = shitiArray[px];
    initShiti(nextShiti, self); //初始化试题对象
    processModelRealDoneAnswer(nextShiti.done_daan, nextShiti, self);
  ...
  storeModelRealLastShiti(px, self); //存储最后一题的状态
...
function processTapLianxiAnswer(midShiti, preShiti, nextShiti, px, current, circular, shitiArray, self) {
  let myFavorite = midShiti.favorite == undefined ? '0' : midShiti.favorite;
  let myCurrent = current;
...
module.exports = {
  ...
  processTapModelRealAnswer: processTapModelRealAnswer
}